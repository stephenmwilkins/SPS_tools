...
def lines(input_dir, model, lines = []):


    log10M_cluster = 8.

    lam, cID, intrinsic, emergent = np.loadtxt('{0}/{1}.lines'.format(input_dir,model), dtype = str, delimiter='\t', usecols = (0,1,2,3)).T
...
def continuum(input_dir, model):
    ...
    lam, incident, transmitted, nebular, total, linecont  = np.loadtxt('{0}/{1}.cont'.format(input_dir, model), delimiter='\t', usecols = (0,1,2,3,4,8)).T # 1 = incident, 2 = transmitted, 3 = nebular, 4 = total, 8 = contribution of lines to total
