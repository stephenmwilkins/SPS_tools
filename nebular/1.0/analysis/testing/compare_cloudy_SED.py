...
Lnu = np.load('../../stellar/SSP/outputs/'+SPS+'/'+IMF+'/'+str(Z)+'_'+str(log10age)+'.npy')  # W s^-1 Hz^-1
...
model = '../RunGrid/Simple/coutputs/C17.00/'+SPS+'/'+IMF+'/'+str(Z)+'_'+str(log10age)
...
# model = '../RunGrid/Full/coutputs/C17.00/'+SPS+'/'+IMF+'/'+'_'.join([str(Z),str(log10age),str(log10U),str(log10n_H),str(d2m),str(CO)])
# lam, intrinsic_stellar, stellar, nebular, linecont = np.loadtxt(model+'.cont', delimiter='\t', usecols = (0,1,2,3,8)).T # 1 = incident, 2 = transmitted, 3 = nebular, 4 = total, 8 = contribution of lines to total
