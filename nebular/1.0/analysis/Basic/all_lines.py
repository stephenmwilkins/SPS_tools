...
# --- output line list
line_list = {'ID': ID, 'lam': lam, 'element': element, 'R': R}
pickle.dump(line_list, open('line_list.p','w'))
...
for e in include_elements:
    ...
    if len(ID[s])>0:
        ...
        for i, id in enumerate(ID[s]):
            ax.annotate(id, (lam[s][i]*10**(0.008), R[s][i]-0.03), size = 3, alpha = 0.5, family = 'sans-serif')
...
plt.figtext(left+width/2., bottom+height+0.01, r'${\rm '+model_label+'}$', size = 7, alpha = 0.6, ha = 'center', va = 'bottom')
...
if default:
    fig_name = 'figures/alllines_'+str(Z)+'.pdf'
else:
    fig_name = 'figures/alllines_'+model+'.pdf'
