...
def BASIClines(SPS, IMF, model, grid = 'Simple', cloudy_version = 'C17.00'):
    ...
    for l,id, intrin, emerg in zip(lam, cID, intrinsic, emergent): 
        ...
        out.lam.append(l)
        out.ID.append(ID)
        out.element.append(e)
        out.emergent.append(emerg)
            
                

    out.lam = np.array(out.lam)
    out.ID = np.array(out.ID)
    out.element = np.array(out.element)
    out.emergent = np.array(out.emergent)
    out.R = out.emergent - out.emergent[(out.ID=='HI4861')] # relative to Hbeta
