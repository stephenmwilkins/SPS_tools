...
for log10tmax, ls in zip([7., 8.], ['-','-.']):
    ...
    ax.plot(grid['log10Z'], np.log10(np.array(line_luminosities)), label = r'$\rm t_{SF}='+"{0:g}".format(10**(log10tmax-6.))+r'\ Myr$', c='0.5', ls=ls)
...
figname = 'figures/SFR_'+label+'_Z.pdf'
print(figname)
fig.savefig(figname)
