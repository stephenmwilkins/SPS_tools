...
ax_EW = fig.add_axes((left, bottom, width, height))

bottom += height

ax_L = fig.add_axes((left, bottom, width, height))

ax_EW.fill_between([-3.0,-2.0], [-7,-7], [5.,5.], color='k',alpha=0.025)
ax_L.fill_between([-3.0,-2.0], [36,36], [44,44], color='k',alpha=0.025)
...
labels['NII6583'] = r'\regular{[NII]6583}'
labels['SII6731-SII6716'] = r'\regular{[SII]6716,6731}'
...
for log10tmax in [7.]:
    ...
    for il, line, ls in zip(range(len(lines)), lines, ['-','-.','--',':']*3):
        ...
        ax_EW.plot(grid['log10Z'], np.array(EWs), label = r'$\rm '+labels[label]+r'$', c=c, ls=ls, lw=1)
        ax_L.plot(grid['log10Z'], np.array(line_luminosities), label = r'$\rm '+labels[label]+r'$', c=c, ls=ls, lw=1)

        
# ax.legend(loc = 'center left', fontsize=7, bbox_to_anchor = (1.0, 0.5))

ax_EW.legend(loc = 'lower center', fontsize=6, handlelength=2.5, labelspacing=0.2)

ax_EW.set_xlim([-5.0, -1.4])
ax_EW.set_ylim([-3.5, 3.6])


ax_EW.set_xlabel(r'$\rm\log_{10}(Z)$')
ax_EW.set_ylabel(r'$\rm\log_{10}(EW/\AA)$')

ax_L.set_xlim([-5.0, -1.4])
ax_L.set_ylim([37., 43.2])

ax_L.set_ylabel(r'$\rm\log_{10}(L/erg\ s^{-1}\ M_{\odot}^{-1})$')

ax_L.set_xticklabels([])

figname = 'figures/lines_combined.pdf'
